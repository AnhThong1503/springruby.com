...
public class ProductController {
	...
	public String listAll(Model model) {
		return "redirect:/products/page/1?sortField=name&sortDir=asc&category=0";
	}
	...
	public String saveProduct(Product product,
			...
			@AuthenticationPrincipal RubyUserDetails loggedUser, RedirectAttributes ra) throws IOException {
	...
	public String deleteProduct(@PathVariable(name = "id") Integer id, Model model,
		...
		try {
			...
			FileUploadUtil.removeDir(uploadDirExtra);
	...
	public String editProduct(@PathVariable("id") Integer id, Model model, RedirectAttributes ra,
			@AuthenticationPrincipal RubyUserDetails loggedUser) {
		try {
			Product product = productService.get(id);
			List<Brand> listBrands = brandService.listAll();
			Integer numberOfExistingExtraImages = product.getImage().size();
